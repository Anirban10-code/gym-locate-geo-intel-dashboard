...
async function fetchSingleZone(
    ...
    try {
        ...
        // (Places without a status are usually public nodes like parks/stations, so we keep them)
        return rawPlaces.filter(p => !p.businessStatus || p.businessStatus === 'OPERATIONAL');
    } catch (error) {
